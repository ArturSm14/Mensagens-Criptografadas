...
async def message():
    async  with websockets.connect("ws://192.168.0.12:8765") as socket:
        ...
        def get_input(prompt):
            # Usa a função input de uma maneira segura
            try:
                return input(prompt)
        ...
        for _ in range(3):
            ...
            if user in users:
                for _ in range(3):
                    ...
                    if password is not None and users[user] == password.strip():
